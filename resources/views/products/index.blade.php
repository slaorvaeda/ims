    ...
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            <h2 class="font-heading font-bold text-2xl text-slate-800 dark:text-white leading-tight">
                {{ __('Product Master Catalog') }}
            </h2>
            @if(auth()->user()->hasPermission('products.create'))
            <a href="{{ route('products.create') }}" class="px-5 py-2.5 bg-[#FF5A36] hover:bg-[#E04826] text-white text-sm font-semibold rounded-2xl transition-all duration-150 flex items-center gap-2 shadow-lg shadow-[#FF5A36]/20">
    ...
    <div x-data="{ showExcelTools: false }" class="space-y-6">
        ...
        <div class="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-[2rem] shadow-sm">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <form method="GET" action="{{ route('products.index') }}" class="flex-1 flex flex-col sm:flex-row gap-4">
                    <div class="relative flex-1">
                        <input 
                            type="text" 
                            name="search" 
                            value="{{ $search }}" 
                            placeholder="Search products by Product ID, Name, or SKU..." 
                    ...
                    @if ($search)
                        <a href="{{ route('products.index') }}" class="px-5 py-3.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-semibold rounded-2xl text-sm text-center hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
                            Clear
                        </a>
                    @endif
                </form>

                @if(auth()->user()->hasPermission('products.import') || auth()->user()->hasPermission('products.export'))
        ...
        <div x-show="showExcelTools" 
            ...
            <div class="space-y-3">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Download all products matching the current search criteria as an Excel-compatible CSV file.
                </p>
                <a href="{{ route('products.export', ['search' => request('search')]) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#FF5A36] hover:bg-[#E04826] text-white text-xs font-semibold rounded-xl transition-all shadow-md shadow-[#FF5A36]/20">
            ...
            <div class="space-y-3 col-start-auto">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400">
                    Upload an Excel/CSV file with columns: <strong>Product ID, Product Name, SKU, FSN, ASIN</strong>. Existing records with same SKU or Product ID will be updated.
                </p>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
                        @forelse ($products as $product)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/20 text-slate-700 dark:text-slate-300 transition-colors">
                                <td class="py-4.5 px-6 font-semibold">{{ $product->id }}</td>
                                ...
                                <td class="py-4.5 px-6">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-xs">{{ $product->updated_by ?? 'System' }}</span>
                                        <span class="text-[10px] text-slate-400">{{ $product->updated_at->diffForHumans() }}</span>
                                    </div>
                                </td>
                                <td class="py-4.5 px-6 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        @if(auth()->user()->hasPermission('products.create'))
                                        <a href="{{ route('products.edit', $product->id) }}" class="p-2 text-slate-400 hover:text-[#FF5A36] hover:bg-orange-50 dark:hover:bg-slate-800 rounded-xl transition-all" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                            </svg>
                                        </a>
                                        <form method="POST" action="{{ route('products.destroy', $product->id) }}" onsubmit="return confirm('Are you sure you want to delete this product? All related purchase, inward, sale, and dispatch records will be deleted.');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-slate-800 rounded-xl transition-all" title="Delete">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <div class="p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-[2rem] text-center shadow-sm">
            ...
            <p class="text-sm font-semibold text-slate-500 dark:text-slate-400">Viewing the product catalog list is restricted.</p>
        </div>
        @endif
    </div>
</x-app-layout>
