<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between w-full">
            ...
            <a href="{{ route('purchases.create') }}" class="px-5 py-2.5 bg-[#FF5A36] hover:bg-[#E04826] text-white text-sm font-semibold rounded-2xl transition-all duration-150 flex items-center gap-2 shadow-lg shadow-[#FF5A36]/20">
    ...
    <div x-data="{ showExcelTools: false }" class="space-y-6 no-print">
        ...
        <div class="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-[2rem] shadow-sm">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                @if(auth()->user()->hasPermission('purchases.import') || auth()->user()->hasPermission('purchases.export'))
        ...
        <div x-show="showExcelTools" 
             ...
             x-cloak>
            <!-- Export Section -->
            @if(auth()->user()->hasPermission('purchases.export'))
            <div class="space-y-3">
                ...
                <a href="{{ route('purchases.export', ['search' => request('search')]) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#FF5A36] hover:bg-[#E04826] text-white text-xs font-semibold rounded-xl transition-all shadow-md shadow-[#FF5A36]/20">
            ...
            <!-- Import Section -->
            @if(auth()->user()->hasPermission('purchases.import'))
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 text-sm">
                        @forelse ($purchases as $purchase)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/20 text-slate-700 dark:text-slate-300 transition-colors">
                                ...
                                <td class="py-4.5 px-6 font-bold text-[#FF5A36]">₹{{ number_format($purchase->amount, 2) }}</td>
                                <td class="py-4.5 px-6">
                                ...
                                <td class="py-4.5 px-6 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        @if(auth()->user()->hasPermission('purchases.create'))
                                        <a href="{{ route('purchases.edit', $purchase->id) }}" class="p-2 text-slate-400 hover:text-[#FF5A36] hover:bg-orange-50 dark:hover:bg-slate-800 rounded-xl transition-all" title="Edit">
