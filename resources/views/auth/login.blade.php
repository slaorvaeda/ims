...
<body class="min-h-screen flex items-center justify-center p-4 md:p-6 lg:p-8">
    <main class="w-full max-w-7xl bg-white rounded-3xl overflow-hidden shadow-xl border border-gray-100 flex flex-col lg:flex-row min-h-[90vh]">
        ...
        <!-- Left Side: Login Form (approx 45%) -->
        <section class="w-full lg:w-[47%] p-8 md:p-12 lg:p-16 flex flex-col justify-between soft-gradient-bg min-h-[600px] lg:min-h-0">
            ...
            <div class="my-auto max-w-md w-full mx-auto py-8">
                <h2 class="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight font-heading mb-2">Welcome Back!</h2>
                <p class="text-slate-500 text-sm mb-8">Please enter log in details below</p>

                <!-- Status Check -->
                @if (session('status'))
                    <div class="mb-4 text-sm font-medium text-emerald-600 bg-emerald-50 border border-emerald-100 p-3 rounded-xl">
                        {{ session('status') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div class="flex justify-end text-xs mt-1">
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-slate-700 hover:text-black hover:underline font-medium transition-all">
                                Forget password?
                            </a>
                        @endif
                    </div>
            ...
            <div class="text-center text-xs text-slate-500">
                ...
                <a href="{{ route('register') }}" class="font-bold text-slate-800 hover:underline">Sign Up</a>
            </div>
        </section>
