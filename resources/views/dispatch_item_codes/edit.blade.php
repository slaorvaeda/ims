<x-app-layout>
    <x-slot name="header">
        <h2 class="font-heading font-bold text-2xl text-slate-800 dark:text-white leading-tight">
            {{ __('Edit Dispatch Entry #') }}{{ $dispatchItemCode->id }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-[2.5rem] p-6 sm:p-8 shadow-sm">
            <form method="POST" action="{{ route('dispatch-item-codes.update', $dispatchItemCode->id) }}" class="space-y-6">
                @csrf
                @method('PUT')

                <!-- UID / Serial Selection -->
                <div>
                    <label for="uid_select" class="block text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">UID / Serial Code</label>
                    <select 
                        id="uid_select" 
                        name="uid" 
                        required 
                    ...
                    >
                        <!-- Include current UID being edited -->
                        <option value="{{ $dispatchItemCode->uid }}" data-product-id="{{ $dispatchItemCode->product_id }}" selected>
                            {{ $dispatchItemCode->uid }} (Current)
                        </option>
                        @foreach ($availableInwardItems as $item)
                            @if ($item->uid !== $dispatchItemCode->uid)
                ...
                <div>
                    ...
                    >
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}" {{ $dispatchItemCode->product_id == $product->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    >
                        <option value="Sold" {{ old('status', $dispatchItemCode->status) == 'Sold' ? 'selected' : '' }}>Sold</option>
                        <option value="Dispatched" {{ old('status', $dispatchItemCode->status) == 'Dispatched' ? 'selected' : '' }}>Dispatched</option>
                        <option value="Damaged Out" {{ old('status', $dispatchItemCode->status) == 'Damaged Out' ? 'selected' : '' }}>Damaged Out</option>
                    </select>
                    @error('status')
                        <p class="text-rose-500 text-xs mt-2 ml-1">{{ $message }}</p>
                    @enderror
