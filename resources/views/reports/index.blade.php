        ...
        <div class="border-b border-slate-200 dark:border-slate-800/80">
            <nav class="flex space-x-8" aria-label="Tabs">
                ...
                <a href="{{ route('reports.index', array_merge(request()->query(), ['tab' => 'stock'])) }}" 
                   class="border-b-2 py-4 px-1 text-sm font-semibold transition-all flex items-center gap-2 {{ $activeTab === 'stock' ? 'border-[#FF5A36] text-[#FF5A36] dark:text-[#FF5A36] dark:border-[#FF5A36]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-300' }}">
                ...
                <a href="{{ route('reports.index', array_merge(request()->query(), ['tab' => 'inward'])) }}" 
                   class="border-b-2 py-4 px-1 text-sm font-semibold transition-all flex items-center gap-2 {{ $activeTab === 'inward' ? 'border-[#FF5A36] text-[#FF5A36] dark:text-[#FF5A36] dark:border-[#FF5A36]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-300' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                ...
                <a href="{{ route('reports.index', array_merge(request()->query(), ['tab' => 'dispatch'])) }}" 
                   class="border-b-2 py-4 px-1 text-sm font-semibold transition-all flex items-center gap-2 {{ $activeTab === 'dispatch' ? 'border-[#FF5A36] text-[#FF5A36] dark:text-[#FF5A36] dark:border-[#FF5A36]' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 dark:text-slate-400 dark:hover:text-slate-300' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
