    ...
    <aside class="hidden md:flex md:flex-col md:fixed md:inset-y-0 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800/80 justify-between transition-all duration-300 z-30" :class="sidebarMinimized ? 'md:w-20' : 'md:w-64'">
        <div class="flex flex-col">
            ...
            <div class="flex p-5 border-b border-slate-100 dark:border-slate-800/50 transition-all duration-300" :class="sidebarMinimized ? 'flex-col items-center gap-3 px-2 py-4' : 'items-center justify-between gap-3'">
                <a href="{{ route('dashboard') }}" @click="if (sidebarMinimized) { $event.preventDefault(); toggleSidebar(); }" class="flex items-center gap-3" :class="sidebarMinimized ? 'cursor-pointer' : ''">
                ...
                <!-- Minimize Toggle Button (Only on desktop, hidden when minimized) -->
                <button x-show="!sidebarMinimized" @click="toggleSidebar()" class="hidden md:block p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50 focus:outline-none transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <!-- User Profile Area -->
        <div class="p-4 border-t border-slate-100 dark:border-slate-800/50" :class="sidebarMinimized ? 'p-2' : 'p-4'">
            @include('layouts.navigation-profile')
        </div>
    </aside>
</div>
